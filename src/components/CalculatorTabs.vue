<template>
  <div class="calculator-tabs">
    <div class="calculator-tabs-wrapper">
      <div @click="setType(BURIAL)" class="calculator-tabs-item"
           :class="($store.state.serviceType === BURIAL ? 'calculator-tabs-item--selected' : '')">
      ...
      <div @click="setType(CREMATION)" class="calculator-tabs-item"
           :class="($store.state.serviceType === CREMATION ? 'calculator-tabs-item--selected' : '')">
    ...
    <div class="calculator-body">
      <div class="calculator-body-title">Ритуальные принадлежности для захоронения:</div>
      <div class="calculator-products-groups">
        <div v-for="productCategory of getProductsCategories" :key="productCategory._id"
          ...
          <div class="calculator-products-groups__image"
               :class="($store.state.orderList[createIndex(productCategory._id)] ? 'active' : '')">
            <img
                v-if="$store.state.orderList[createIndex(productCategory._id)] && $store.state.orderList[createIndex(productCategory._id)].image"
...
168 185 29 76 24 186 -10 258 -34 69 -108 138 -175 163 -74 28 -180 27 -253 0z"
          ...
          <div>
            ...
            <calculator-select
                :value="($store.state.orderList[createIndex(productCategory._id)] ? $store.state.orderList[createIndex(productCategory._id)]._id : '')"
                :items="productCategory.products"
                :title="productCategory.title"
                @selectItem="(_id)=>this.changeSelect({_id:_id, type:PRODUCT, index: createIndex(productCategory._id)})"
            />
            <div v-if="$store.state.orderList[createIndex(productCategory._id)]"
                 class="calculator-products-groups__price-wrapper"><span
                class="calculator-products-groups__price">
              Цена: <span>{{
                ($store.state.orderList[createIndex(productCategory._id)] && $store.state.orderList[createIndex(productCategory._id)].price ? $store.state.orderList[createIndex(productCategory._id)].price.toLocaleString('ru') : '')
              }} ₽</span></span>
              <span @click="removeFromOrder($store.state.orderList[createIndex(productCategory._id)]._id)"
      ...
      <div class="calculator-products-without-group">
        ...
        168 185 29 76 24 186 -10 258 -34 69 -108 138 -175 163 -74 28 -180 27 -253 0z"
          ...
          <div>
            <calculator-checkbox :item="product"
                                 @selectItem="(_id)=>this.changeSelect({_id:_id, type:PRODUCT, index: createIndex(product._id)})"
            />
            <div class="calculator-products-without-group__price-wrapper"><span
                class="calculator-products-without-group__price">
                      Цена: <span>{{ product.price.toLocaleString('ru') }} ₽</span></span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="calculator-body">
      ...
      <div class="calculator-services-group">
        <div v-for="serviceCategory of getServicesCategories" :key="serviceCategory._id"
          ...
          <calculator-select :items="serviceCategory.services"
                             :title="serviceCategory.title"
                             @selectItem="(_id)=>this.changeSelect({_id:_id, type:SERVICE, index: createIndex(serviceCategory._id)})"
          />
        </div>
      </div>
      <div class="calculator-services-without-group">
        <div v-for="service of getServicesWithOutCategory" :key="service._id"
             class="calculator-services-without-group__item">
          <div>
            <calculator-checkbox :item="service"
                                 @selectItem="(_id)=>this.changeSelect({_id:_id, type:SERVICE, index: createIndex(service._id)})"
            />
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...mapGetters(['getProductsCategories', 'getServicesCategories', 'getProductsWithOutCategory', 'getServicesWithOutCategory'])
  },
  mounted() {
