<template>
	<p v-if="getOrderList.length === 0" class="calculator-form__list-empty">
		Выберите услуги и мы расчитаем стоимость
	</p>
	<div
		v-if="getOrderList.length !== 0"
		:class="getOrderList.length !== 0 ? 'calculator-form--no-empty' : ''"
	...
	>
		<div class="calculator-form__items">
			...
			<div v-for="item of getOrderList" class="calculator-form__item">
				...
				<div
					...
					class="calculator-form__del-btn"
				></div>
			</div>
		</div>
		<form class="calculator-form">
			...
			<calculator-button
				...
				:title="phone.length < 18 ? 'Не заполнены обязательные поля' : ''"
				>Отправить</calculator-button
			>
		</form>
	</div>
</template>
